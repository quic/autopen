'''
This script is to be executed when running Autopen on a VM. Running Autopen on
a VM is not recommended when using hardware mainly because the hardware needs
to be set up. Autopen works best with a dedicated install.
...
ff_rc = subprocess.call(['sudo', 'apt-get', 'install', '-y', 'ffmpeg'])
if ff_rc != 0:
    print 'INSTALLATION FAILED: Failed to install ffmpeg library, trying alternative option: libav-tools'
    lib_rc = subprocess.call(['sudo', 'apt-get', 'install', '-y', 'libav-tools'])
    if lib_rc != 0:
        ...
        ppa_rc = subprocess.call(['sudo', 'add-apt-repository', 'ppa:mc3man/trusty-media'])
        if ppa_rc != 0:
            ...
            if up_rc != 0:
                ...
                ffm_rc = subprocess.call(['sudo', 'apt-get', 'install', 'ffmpeg'])
                if ffm_rc != 0:
                    print 'INSTALLATION FAILED: Could not install ffmpeg package. Functionality may be hindered and/or Autopen may not be able to run'
...
libraries_rc = subprocess.call(['sudo', 'apt-get', 'install', '-y', 'python3-pip', 'build-essential', 'git', 'python-dev','libsdl2-dev', 'libsdl2-image-dev', 'libsdl2-mixer-dev',\
...
if libraries_rc != 0:
    ...
    ch_rc = subprocess.call(['sudo', 'pip3', 'install', 'virtualenv', 'setuptools'])
    if ch_rc != 0:
        ...
        # containing their own copy of python, pip
        x_rc = subprocess.call(['virtualenv', '--no-site-packages', 'kivyinstall'])
        if x_rc != 0:
            ...
            if enter_rc != 0:
                ...
                cython_rc = subprocess.call(['pip3', 'install', 'Cython==0.23'])
                if cython_rc != 0:
                    ...
                    kivy_rc = subprocess.call(['pip3', 'install', 'kivy'])
                    if kivy_rc != 0:
